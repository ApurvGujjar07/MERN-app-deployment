...
if (!formData.imageLink) return toast.error('Image is required'), false;
...
try {
  ...
  const payload = {
    ...
    isFeaturedPost: formData.isFeaturedPost,
  };
...
return ( <div className="flex-grow bg-slate-50 dark:bg-dark py-10"> <div className="flex justify-center mb-6"> <div className="flex items-center space-x-4 w-[32rem]">
<img
src={isDarkMode ? navigateBackWhiteIcon : navigateBackBlackIcon}
onClick={() => navigate(-1)}
className="h-5 w-10 cursor-pointer"
/> <h2 className="text-xl font-semibold">Create Blog</h2> </div> </div>
  ...
  <div className="flex justify-center">
    ...
    >
      <input name="title" placeholder="Title" onChange={handleInputChange} className="w-full p-2 border rounded" />

      <textarea name="description" placeholder="Content" onChange={handleInputChange} className="w-full p-2 border rounded" />

      <input name="authorName" placeholder="Author" onChange={handleInputChange} className="w-full p-2 border rounded" />
